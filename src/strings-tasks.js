...
function sumOfCodes(str) {
  ...
  return str.split('').reduce((el1, el2) => el1 + el2.charCodeAt(), 0);
}
...
function countVowels(str) {
  ...
  const res = str.split('');
  return res.filter((el) => vowelsArr.includes(el)).length;
}
...
function isPalindrome(str) {
  const cleanLower = str.replace(/[^a-zA-Z]/g, '').toLowerCase();
  const reverse = cleanLower.split('').reverse().join('');
  return cleanLower === reverse;
}
...
function findLongestWord(sentence) {
  ...
  const sortedArr = splittedArr
    .map((el) => {
      return el.length;
    })
    .sort((a, b) => b - a);
  return splittedArr.find((el) => el.length === sortedArr[0]);
}
